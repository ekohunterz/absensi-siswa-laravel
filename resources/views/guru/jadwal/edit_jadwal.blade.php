...
@Section('container')
    ...
    <form action="/admin/jadwal/{{ $data_jadwal->id }}" method="POST">
        @method('put')
        @csrf
        <div class="mb-3">
            ...
            <select name="kelas_id" class="form-select @error('kelas_id') is-invalid @enderror" id="kelas_id"
                ...
                @foreach ($data_kelas as $kelas)
                    @if (old('kelas_id', $data_jadwal->kelas->id) == $kelas->id)
        ...
        <div class="mb-3">
            ...
            <input type="time" name="jam_mulai" class="form-control @error('jam_mulai') is-invalid @enderror"
                id="jam_mulai" placeholder="jam_mulai" value="{{ old('jam_mulai', $data_jadwal->jam_mulai) }}">
        </div>
        <div class="mb-3">
            ...
            <input type="time" name="jam_selesai" class="form-control @error('jam_selesai') is-invalid @enderror"
                id="jam_selesai" placeholder="jam_selesai" value="{{ old('jam_selesai', $data_jadwal->jam_selesai) }}">
        </div>
        <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">Keterangan</label>
            <input type="text" name="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                id="keterangan" placeholder="keterangan" value="{{ old('keterangan', $data_jadwal->keterangan) }}">
        </div>
        <div class="col-auto">
