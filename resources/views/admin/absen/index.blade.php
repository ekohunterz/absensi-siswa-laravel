...
@Section('container')
    <div class="col-12">
        <div class="card my-4">
            ...
            <form action="/admin/absen" method="get">
                <div class="mx-3 my-2">
                            ...
                            <div class="input-group input-group-outline my-3">
                                <select class="form-control" name="jadwal_id" id="jadwal_id">
                                    ...
                                    @foreach ($jadwals as $jadwal)
                                        @if (request('jadwal_id') == $jadwal->id)
                                            <option value="{{ $jadwal->id }}" selected>{{ $jadwal->mapel->nama }}</option>
                                        @else
                                            <option value="{{ $jadwal->id }}">{{ $jadwal->mapel->nama }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            $('#kelas_id').on('change', function() {
                ...
                document.getElementById('jadwal_id').removeAttribute('disabled');
                if (kelas_id) {
                    $.ajax({
                        url: '/getjadwal/' + kelas_id,
                        type: "GET",
                        dataType: "json",
                        success: function(data) {
                            ...
                            $.each(data, function(key, value) {
                                $('#jadwal_id').append('<option value="' + value.id +
                                    '">' + value.nama + ' - ' + value.hari +
                                    '</option>');
                            });
                        }
                    });
                    ...
                    $('#jadwal_id').empty();
                    $('#jadwal_id').append('<option value="" hidden>Pilih Jadwal</option>');
                }
            });
        });
    </script>
